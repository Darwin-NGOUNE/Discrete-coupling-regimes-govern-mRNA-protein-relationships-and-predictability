# ==============================================================================
# SCRIPT: generate_75th_percentile_scatterplots_Proc1.R
# PURPOSE: Generate 75th Percentile Scatterplot PDFs for Procedure 1 (Subset Models)
#          including both:
#            1. Single-Page PDFs (Baseline 4-panel, 3-Models 12-panel)
#            2. NEW Strict 3-Page Landscape 1x4 PDFs (Page 1: Baseline, Page 2: Mastery 50, Page 3: Protein)
#          WITH PROPER CCl4 SUBSCRIPTING (CCl[4]) and mouse status color alignment!
# ==============================================================================
...
# ------------------------------------------------------------------------------
# STEP 5: GENERATE SCATTERPLOT PDFS FOR PROC 1 (SUBSET MODELS)
# Generates BOTH:
#   1. Single-Page PDFs (Baseline 4-panel, 3-Models 12-panel)
#   2. NEW Strict 3-Page Landscape 1x4 PDFs with CCl4 subscript!
# ------------------------------------------------------------------------------
generate_proc1_75th_pdfs <- function(dir_short, prime_files, m50_files) {
  ...
  if (length(base_panels) > 0) {
    top_grob  <- grid::textGrob(top_expr_base, gp = grid::gpar(fontsize = 15, col = "#1B4F72", fontfamily = "sans"))
  ...
  if (length(panels_12) == 12) {
    top_grob <- grid::textGrob(top_expr_3m, gp = grid::gpar(fontsize = 16, col = "#1B4F72", fontfamily = "sans"))
  ...
  if (length(base_panels) == 4) {
    ...
    # Page 1: Baseline with unified centered tight header (Title + | + Mouse status legend)
    ...
    header_grob1 <- cowplot::as_grob(
      cowplot::ggdraw() +
        cowplot::draw_label(top_expr_pg1, x = 0.470, y = 0.5, hjust = 1, size = 22, fontface = "bold") +
    ...
    # Page 2: Mastery 50
    grid::grid.newpage()
    top_grob2 <- grid::textGrob(top_expr_pg2, gp = grid::gpar(fontsize = 24, fontface = "bold", col = "black", fontfamily = "sans"))
    g_m50     <- gridExtra::arrangeGrob(grobs = panels_m50, ncol = 4, nrow = 1, top = top_grob2)
    grid::grid.draw(g_m50)
    
    # Page 3: Protein
    grid::grid.newpage()
    top_grob3 <- grid::textGrob(top_expr_pg3, gp = grid::gpar(fontsize = 24, fontface = "bold", col = "black", fontfamily = "sans"))
    g_prot    <- gridExtra::arrangeGrob(grobs = panels_prot, ncol = 4, nrow = 1, top = top_grob3)
    grid::grid.draw(g_prot)
    
    dev.off()
    cat("  -> Saved NEW STRICT 3-PAGE 1x4 PDF (3 Pages Exactly):", pdf_3p, "\n")
  }
}

# ------------------------------------------------------------------------------
# STEP 6: EXECUTE FOR RICHTUNG 1 AND RICHTUNG 2
# ------------------------------------------------------------------------------
r1_prime_files <- c(
...
generate_proc1_75th_pdfs("Richtung1_Train_BDL_Test_CCl4", r1_prime_files, r1_m50_files)
generate_proc1_75th_pdfs("Richtung2_Train_CCl4_Test_BDL", r2_prime_files, r2_m50_files)

cat("\nPROCEDURE 1 (SUBSET MODELS) ALL PDFS INCLUDING NEW 3-PAGE 1x4 FORMAT SUCCESSFULLY GENERATED!\n")
