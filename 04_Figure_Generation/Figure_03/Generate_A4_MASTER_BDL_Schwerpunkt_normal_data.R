# ==============================================================================
# SCRIPT: Generate_A4_MASTER_BDL.R
...
# AESTHETICS: NO GRIDS, NO BOLD TEXT. Text sizes are aggressively miniaturized
#             (size ~8 to 12) so all 13 plots fit onto one single A4 portrait page.
# ==============================================================================
...
print("Loading Cloud (Wolken) Raw DT for specific scatters...")
...
p1_pca_rna  <- generate_master_pca(DT_BDL_BatchCorrected, "GeneCount", "RNA", "Sham Vehicle", "BDL Vehicle") + labs(tag = "A") + theme(plot.tag = element_text(face = "bold", size = 20)) + theme(legend.position = "none")
p1_pca_prot <- generate_master_pca(DT_BDL_BatchCorrected, "Protein_Raw", "Protein", "Sham-Vehicle", "BDL Vehicle") + theme(legend.position = "none")
...
p2_volc_rna  <- plot_master_volcano(calculate_combat_volcano(DT_BDL_BatchCorrected, "GeneCount"), "RNA") + labs(tag = "B") + theme(plot.tag = element_text(face = "bold", size = 20)) + theme(legend.position = "none")
p2_volc_prot <- plot_master_volcano(calculate_combat_volcano(DT_BDL_BatchCorrected, "Protein_Raw"), "Protein") + theme(legend.position = "none")

# -------------------------------------------------------------------------
# 4. STEP 3: DIPA & CLOUDS
# -------------------------------------------------------------------------
# -------------------------------------------------------------------------
# 4. STEP 3: DIPA & CLOUDS — VERSION SCHWERPUNKT
# -------------------------------------------------------------------------

Cloud_Metrics_BDL <- bdl_DT[, {
  
  # --- Séparer Control et Disease pour CE gène (on est déjà groupé par GeneProtein) ---
...
# --- Points extrêmes par DiPa group ---
...
for(g in 1:8) {
  ...
  all_x <- c(
    p_dat$GeneCount,
    met$Schwerpunkt_RNA_C,
    met$Schwerpunkt_RNA_D
  )
  all_y <- c(
    p_dat$Protein_Raw,
    met$Schwerpunkt_Prot_C,
    met$Schwerpunkt_Prot_D
  )
  ...
  # --- Create a local Schwerpunkt data frame for this specific gene ---
  df_sp <- data.frame(
  ...
  # --- Calculate Line Parameters (Through the two points) ---
  ...
  cp <- ggplot(p_dat, aes(x=GeneCount, y=Protein_Raw, color=DiseaseGroup)) +
    ...
    theme(
      ...
      plot.title=element_text(color=col, size=T_TITLE - 4, hjust=0.5, face="bold"), # Scale down a bit for the small subplots
      plot.subtitle=element_text(size=T_SUB - 2, color="black", hjust=0.5),
      axis.title=element_text(size=T_TICKS - 2),
      axis.text=element_text(size=8, color="black"), # increased from 6 to 8 for better readability
      axis.ticks=element_line(color="black", linewidth=0.8),
  ...
  # --- Ligne Noire Longue (Traversant les deux points) ---
  if (abs(dx) < 1e-6) {
    # Cas Vertical
  ...
  # --- Points Schwerpunkt par-dessus (Smaller crosses) ---
  cp <- cp +
...
layout_master <- p1_pca_rna + p1_pca_prot + p2_volc_rna + p2_volc_prot +
  ...
  plot_annotation(
    ...
    subtitle = paste0(
      ...
      "** Points:  Blue = Control (Sham) | Red = Disease (BDL) **\n",
...
ggsave("C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/New_Data/A4_MASTER_BDL_Combined_Schwerpunkt_normal_data.pdf", plot = layout_master, width = 11.7, height = 14.0, device = cairo_pdf)
print("SUCCESS! File A4_MASTER_BDL_Combined_Schwerpunkt_normal_data.pdf was saved!")

# -------------------------------------------------------------------------
# 6. SAVE ISOLATED PLOTS (3 SEPARATE FILES)
# -------------------------------------------------------------------------
print("Saving isolated plots for BDL...")

# MODIFIED: Removed global titles, set subplot titles to "RNA" and "Protein", and further increased text sizes (Very large)
...
ggsave("C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/New_Data/Isolated_PCA_BDL.pdf", plot = layout_pca, width = 10, height = 5, device = cairo_pdf)
...
ggsave("C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/New_Data/Isolated_Volcano_BDL.pdf", plot = layout_volcano, width = 10, height = 5, device = cairo_pdf)
...
design_isolated_dipa <- "
  ...
  EEEEEEMMMNNN
"
layout_dipa_isolated <- (p3_dipa_iso + labs(tag="A") + theme(plot.tag = element_text(face = "bold", size = 34))) + 
  (clouds_list_iso[["1"]] + labs(tag="B") + theme(plot.tag = element_text(face = "bold", size = 34))) + clouds_list_iso[["2"]] + clouds_list_iso[["3"]] + 
...
ggsave("C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/New_Data/Isolated_DiPa_Wolken_BDL.pdf", plot = layout_dipa_isolated, width = 19, height = 17.5, device = cairo_pdf)

print("SUCCESS! Isolated plots for BDL were saved.")
