...
p1_pca_rna  <- generate_master_pca(DT_BDL_BatchCorrected, "GeneCount", "RNA", "Sham Vehicle", "BDL Vehicle") + labs(tag = "A") + theme(plot.tag = element_text(face = "bold", size = 22)) + theme(legend.position = "none")
...
p3_dipa_iso <- ggplot(Combined, aes(x = log2G, y = log2P, color = CloudCategory)) +
  ...
  annotate("text", x =  4.0,  y = -3.5,  label = "7", size = 10, color = "grey50", fontface = "bold") +
  geom_point(data = extreme_pts, aes(x = log2G, y = log2P),
             shape = 18, size = 4.5, color = "black", inherit.aes=FALSE) +
  geom_text_repel(
    data = extreme_pts,
    aes(label = PlotMarker),
    size = 6.5,
...
layout_dipa_isolated <- (p3_dipa_iso + labs(tag="A") + theme(plot.tag = element_text(face = "bold", size = 32))) + 
  (clouds_list_iso[["1"]] + labs(tag="B") + theme(plot.tag = element_text(face = "bold", size = 32))) + clouds_list_iso[["2"]] + clouds_list_iso[["3"]] + 
