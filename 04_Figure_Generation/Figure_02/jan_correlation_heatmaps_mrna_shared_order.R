# ==============================================================================
# SCRIPT: jan_correlation_heatmaps_mrna_shared_order.R
# LOCATION: C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/New_Data/jan_correlation_heatmaps_mrna_shared_order.R
...
p_bdl_mrna <- ggplot(dt_r1_mrna, aes(x = x_rank, y = y_rank, fill = Correlation)) +
  ...
  scale_fill_gradient2(
    ...
    name = expression(bold(rho[BP] ~ "(BDL mRNA)"))
  ) +
  labs(
    title = expression(bold("A. Datensatz 1: BDL mRNA (") * bold(R[mRNA]^{"(1)"}) * bold(")")),
...
p_ccl4_mrna <- ggplot(dt_r2_mrna, aes(x = x_rank, y = y_rank, fill = Correlation)) +
  ...
  scale_fill_gradient2(
    low = "#1B4F72", mid = "#FFFFFF", high = "#922B21",
    midpoint = 0, limits = c(-1, 1),
    name = expression(bold(rho[BP] ~ (CCl[4] ~ mRNA)))
  ) +
  labs(
    title = expression(bold("B. Datensatz 2: ") * bold(CCl[4]) * bold(" mRNA (") * bold(R[mRNA]^{"(2)"}) * bold(")")),
    subtitle = "Identical Shared mRNA Order"
  ) +
  theme_heatmap()

# Heatmap C: Difference (Delta R mRNA)
p_delta_mrna <- ggplot(dt_delta_mrna, aes(x = x_rank, y = y_rank, fill = Delta)) +
  ...
  labs(
    title = expression(bold("C. Differenz-Heatmap: ") * bold(Delta * R[mRNA] == R[mRNA]^{"(1)"} - R[mRNA]^{"(2)"})),
...
# ------------------------------------------------------------------------------
# 5. SAVE HIGH-RES TRIPTYCH & PNG PREVIEW
# ------------------------------------------------------------------------------
g_triptych_mrna <- gridExtra::arrangeGrob(
  p_bdl_mrna, p_ccl4_mrna, p_delta_mrna, ncol = 3,
  top = textGrob("mRNA Korrelations-Heatmaps mit identischer Genreihenfolge (N = 943 Gene)", 
...
cairo_pdf(pdf_triptych_mrna, width = 18, height = 7.2)
