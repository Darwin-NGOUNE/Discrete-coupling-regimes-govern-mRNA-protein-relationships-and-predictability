...
# PURPOSE: Generate 4-Models Pearson Correlation Boxplots for Procedure 1 (Train BDL, test CCl4)
...
extract_model_data <- function(filepath, model_type, dipa_label) {
  ...
  for (v in vars) {
    ...
    if (model_type == "Baseline" && grepl("rna", v_low) && !grepl("plus_rna|combined|mastery", v_low)) {
      obj_target <- env[[v]]
    } else if (model_type == "RF + LASSO" && grepl("protein", v_low) && !grepl("plus_rna|combined|mastery", v_low)) {
      obj_target <- env[[v]]
    } else if (model_type == "Mastery 50" && grepl("protein", v_low) && !grepl("plus_rna|combined", v_low)) {
      obj_target <- env[[v]]
    } else if (model_type == "RF + RF" && (grepl("rf", v_low) || grepl("protein", v_low)) && !grepl("plus_rna|combined", v_low)) {
      obj_target <- env[[v]]
    }
  }
...
run_proc1_4models <- function(direction_title, dir_tag, file_prefix_prime, file_prefix_m50, file_prefix_rfrf, out_filename) {
  dt_list <- list()
  
  for (i in 1:4) {
    f_prime <- file.path(prime_dir, sprintf("%s_%s_sub.RData", file_prefix_prime, group_codes[i]))
    f_m50   <- file.path(m50_dir,   sprintf("%s_%s_prime_sub.RData", file_prefix_m50, group_codes[i]))
    f_rfrf  <- file.path(rf_rf_dir, sprintf("%s_%s_sub.RData", file_prefix_rfrf, group_codes[i]))
  ...
  p <- ggplot(dt_all, aes(x = DiPa_Group, y = Pearson, fill = Model)) +
    ...
    geom_text(data = summary_dt, 
              aes(x = DiPa_Group, y = -1.34, label = label, group = Model),
              position = position_dodge(width = 0.8),
              family = "sans", size = 3.6, color = "black", fontface = "bold", lineheight = 0.82, inherit.aes = FALSE) +
    ...
    coord_cartesian(ylim = c(-1.62, 1.05), clip = "off") +
...
# Generate Direction 1 (Train BDL, test CCl4) - Figure 6 Panel D
out1 <- run_proc1_4models(
  direction_title   = expression(bold("Train BDL, test "*CCl[4])),
  dir_tag           = "Richtung1",
