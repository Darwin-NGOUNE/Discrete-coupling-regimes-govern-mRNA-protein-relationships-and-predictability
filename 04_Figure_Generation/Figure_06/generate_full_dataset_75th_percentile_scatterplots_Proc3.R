...
if (length(base_panels) == 4) {
  ...
  make_proc3_full_header <- function(title_expr) {
    ...
    header_grob <- cowplot::as_grob(
      cowplot::ggdraw() +
        cowplot::draw_label(title_expr, x = 0.450, y = 0.5, hjust = 1, size = 22, fontface = "bold") +
  ...
  g_base       <- gridExtra::arrangeGrob(grobs = base_panels, ncol = 4, nrow = 1, top = make_proc3_full_header(top_expr_pg1))
  grid::grid.draw(g_base)
  
  # Page 2: Mastery 50
  grid::grid.newpage()
  top_expr_pg2 <- bquote(bold("Mastery 50 model: BDL + CCl"[4] * " (all animals)"))
  g_m50        <- gridExtra::arrangeGrob(grobs = panels_m50, ncol = 4, nrow = 1, top = make_proc3_full_header(top_expr_pg2))
  grid::grid.draw(g_m50)
  
  # Page 3: Protein
  grid::grid.newpage()
  top_expr_pg3 <- bquote(bold("Protein model: BDL + CCl"[4] * " (all animals)"))
  g_prot       <- gridExtra::arrangeGrob(grobs = panels_prot, ncol = 4, nrow = 1, top = make_proc3_full_header(top_expr_pg3))
