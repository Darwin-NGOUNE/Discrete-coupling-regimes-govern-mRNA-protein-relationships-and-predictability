...
# PURPOSE: Generate 4-Models Pearson Correlation Boxplots for Procedure 3 (BDL + CCl4)
...
output_file <- file.path(output_dir, "Procedure_3_4Models_Pearson_Merged_Batch.pdf")
...
prime_rfrf_dir <- file.path(prime_dir, "RF_RF")
...
for (i in 1:4) {
  ...
  dt_prot <- extract_model_data(prime_files[i], "RF + LASSO", dipa_groups[i])
  dt_rfrf <- extract_model_data(rfrf_files[i],  "RF + RF",     dipa_groups[i])
...
p <- ggplot(dt_all, aes(x = DiPa_Group, y = Pearson, fill = Model)) +
  ...
  labs(
    title = expression(bold("BDL + "*CCl[4])),
    fill  = "Model:"
  )
...
cat("Procedure 3 4-Models PDF successfully generated:", output_file, "\n")
