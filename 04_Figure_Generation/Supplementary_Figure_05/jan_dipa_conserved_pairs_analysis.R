...
tier_levels <- c("Lost / Noise", "Weak", "Moderate", "High")
tier_colors <- c(
  "High"         = "#27AE60", # Green
  "Moderate"     = "#2980B9", # Blue
  "Weak"         = "#F39C12", # Orange
  "Lost / Noise" = "#E74C3C"  # Red
)
...
build_stacked_plot <- function(dt_master, dir_col, train_name, test_name, cohort_label) {
  ...
  legend_labels <- c(
    "Lost / Noise" = expression(paste("Lost / Noise (", rho[BP] < 0.2, ")")),
    "Weak"         = expression(paste("Weak (", 0.2 <= rho[BP], " < 0.5)")),
    "Moderate"     = expression(paste("Moderate (", 0.5 <= rho[BP], " < 0.8)")),
    "High"         = expression(paste("High (", rho[BP] >= 0.8, ")"))
  )
  ...
  p <- ggplot(agg_dt, aes(x = DiPa_Group, y = Prop, fill = Tier)) +
    ...
    geom_text(data = top_annot, aes(x = DiPa_Group, y = 1.025, label = TopLabel, fill = NULL, size = NULL),
              family = "sans", size = 3.6, fontface = "bold", color = "#1B4F72", lineheight = 0.85, vjust = 0) +
    
    scale_y_continuous(
      labels = scales::percent_format(accuracy = 1),
      name = "Proportion of Conserved Pairs (%)",
    ...
    scale_fill_manual(
      ...
      name = bquote(bold("Conservation Tier in Test (") * .(test_expr) * bold("):")),
      guide = guide_legend(reverse = TRUE, nrow = 1)
    ) +
    theme_bw(base_size = 13, base_family = "sans") +
    theme(
      plot.title = element_text(face = "bold", size = 16, hjust = 0.5, color = "#1B4F72", margin = margin(b = 5)),
      plot.subtitle = element_text(size = 12.5, hjust = 0.5, color = "#566573", margin = margin(b = 12)),
      plot.margin = margin(t = 20, r = 20, b = 20, l = 20),
      axis.title.x = element_blank(),
      axis.text.x  = element_text(face = "bold", size = 12, color = "#2C3E50", margin = margin(t = 6)),
      axis.title.y = element_text(face = "bold", size = 13, color = "#2C3E50"),
      axis.text.y  = element_text(size = 11, color = "#2C3E50"),
      legend.position = "top",
      legend.title    = element_text(face = "bold", size = 11.5, color = "#1B4F72"),
    ...
    labs(
      title = expression(bold("Proportion of Conserved Best-Partner Pairs Across DiPa Groups (") * bold(rho[BP]) * bold(")")),
      subtitle = bquote(bold("Direction: Train ") * .(train_expr) %->% bold(" Test ") * .(test_expr) ~ .(cohort_label) ~ bold("(Bravais-Pearson Correlation ") * bold(rho[BP]) * bold(")"))
    )
...
run_cohort_analysis <- function(bdl_input, ccl4_input, cohort_label, filename_suffix) {
  ...
  cairo_pdf(pdf_d1, width = 11.5, height = 7.8)
  print(p1)
  dev.off()
  if (filename_suffix == "Subset") {
    cairo_pdf(file.path(out_dir, "Proportion_Conserved_Pairs_per_DiPa_Group_Direction1_BDL_to_CCl4_Pearson.pdf"), width = 11.5, height = 7.8)
    print(p1)
    dev.off()
  }
