# ==============================================================================
# SCRIPT: Global_RNA_Landscape_Heatmaps_ALL_Mice.R
# LOCATION: C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/New_Data/
# OBJECTIVE: Generate a clean 2-Page PDF report containing ONLY the Unsupervised RNA Heatmaps
...
pdf_path <- file.path(out_dir, "Global_RNA_Landscape_Heatmaps_ALL_Mice.pdf")
...
draw_annotation_legend_parsed <- function(annotation, annotation_colors, border_color, ...) {
    y = unit(1, "npc")
    text_height = unit(1.0, "grobheight", textGrob("FGH", gp = gpar(fontsize = 12, fontface = "bold", ...)))
    res = gList()
    for (i in names(annotation)) {
        ...
        if (is.character(annotation[[i]]) | is.factor(annotation[[i]])) {
            ...
            yy = y - (1:n - 1) * 1.8 * text_height
            res[[paste(i, "r")]] = rectGrob(x = unit(0, "npc"), 
                y = yy, hjust = 0, vjust = 1, height = 1.3 * text_height, 
                width = 1.3 * text_height, gp = gpar(col = border_color, 
            ...
            for(j in 1:length(labels_parsed)) {
                res[[paste(i, "t", j)]] = textGrob(labels_parsed[[j]], 
                    x = text_height * 1.6, y = yy[j] - 0.65 * text_height, 
                    hjust = 0, vjust = 0.5, gp = gpar(fontface = "bold", fontsize = 11, ...))
            }
            y = y - n * 1.8 * text_height
        }
        else {
            ...
            res[[paste(i, "r")]] = rectGrob(x = unit(0, "npc"), 
                y = yy, hjust = 0, vjust = 1, height = h, width = 1.5 * 
            ...
            res[[paste(i, "r2")]] = rectGrob(x = unit(0, "npc"), 
                ...
                width = 1.5 * text_height, gp = gpar(col = border_color, 
                  fill = NA))
            txt = rev(range(grid.pretty(range(annotation[[i]], 
                na.rm = TRUE))))
            yy = y - c(1, 7) * text_height
            res[[paste(i, "t")]] = textGrob(txt, x = text_height * 
                1.8, y = yy, hjust = 0, vjust = 0.5, gp = gpar(fontface = "bold", fontsize = 10.5, ...))
            y = y - 8 * text_height
        }
        y = y - 1.4 * text_height
    }
...
cairo_pdf(pdf_path, width = 11.5, height = 8.5)

# ==============================================================================
# PAGE 1: BDL UNSUPERVISED RNA HEATMAP (ALL 18 MICE WITH ZWISCHENMÄUSE)
# ==============================================================================
print("Generating Page 1: BDL RNA Heatmap (18 Mice)...")

bdl_heatmap_dt <- BDL_DT[!is.na(GeneCount), .(GeneProtein, GeneCount, MiceInfo, Treatment)]
...
mat_wide_bdl <- dcast(bdl_heatmap_dt, GeneProtein ~ MiceInfo, value.var = "GeneCount")
...
y_label_bdl <- grid::textGrob(paste0("Individual genes N = ", nrow(mat_matrix_bdl)), 
...
# ==============================================================================
# PAGE 2: CCL4 UNSUPERVISED RNA HEATMAP (ALL MICE WITH ZWISCHENMÄUSE)
# ==============================================================================
print("Generating Page 2: CCL4 RNA Heatmap (All Mice with Zwischenmäuse)...")

ccl4_heatmap_dt <- CCL4_DT[!is.na(GeneCount) & MiceInfo != "month12_ccl4_rep6", 
                           .(GeneProtein, GeneCount, MiceInfo, Treatment, TreatmentTime)]
...
mat_wide_ccl4 <- dcast(ccl4_heatmap_dt, GeneProtein ~ MiceInfo, value.var = "GeneCount")
...
y_label_ccl4 <- grid::textGrob(paste0("Individual genes N = ", nrow(mat_matrix_ccl4)), 
                                rot = 90, gp = grid::gpar(fontsize = 14, fontface = "bold"))
...
print(paste("SUCCESS! 2-Page RNA Heatmaps Report saved to:", pdf_path))
