...
pairs.list.3.4.sub <- unique(CCL4_Full[ClusterDiPa %in% c(3, 4), GeneProtein])
...
lasso.rf.pre.full.CD.new_mastery50_DTccl4_cluster_3_4_sub <- pblapply(FUN = protein.regression.complete,
                          pairs.list.3.4.sub,
...
lasso.rf.pre.full.CD.new_mastery50_plus_rna_DTccl4_cluster_3_4_sub <- pblapply(FUN = protein.regression.complete,
                          pairs.list.3.4.sub,
                          dataset = CCL4_Full, # <-- Full dataset to access all 10 hubs
                          weighted = FALSE,
                          grouping = "all",
...
# 4. SAVE RESULTS
save(lasso.rf.pre.full.CD.new_mastery50_DTccl4_cluster_3_4_sub,
     lasso.rf.pre.full.CD.new_mastery50_plus_rna_DTccl4_cluster_3_4_sub,
     file = "Models_mastery50_DTccl4_cluster_3_4_sub.RData")

# 5. EVALUATION AND SUMMARY STATISTICS
# res.m10 <- gof.all.complete(model = lasso.rf.pre.full.CD.new_mastery50_DTccl4_cluster_3_4_sub, prediction.method = "lasso")
# cat("\n=== MASTERY 10 PROTEIN CORRELATIONS ===\n")
# cor_m10 <- sapply(lasso.rf.pre.full.CD.new_mastery50_DTccl4_cluster_3_4_sub, function(x) as.numeric(x$prediction.obj$model$correlation.pearson))
