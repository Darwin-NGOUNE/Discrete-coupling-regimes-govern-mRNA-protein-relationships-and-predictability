...
pairs.list.1.2 <- unique(CCL4_Full[ClusterDiPa %in% c(1, 2), GeneProtein])
...
lasso.rf.pre.full.CD.new_mastery50_DTccl4_cluster_1_2 <- pblapply(FUN = protein.regression.complete,
                          pairs.list.1.2,
...
lasso.rf.pre.full.CD.new_mastery50_plus_rna_DTccl4_cluster_1_2 <- pblapply(FUN = protein.regression.complete,
                          pairs.list.1.2,
                          dataset = CCL4_Full, # <-- Full dataset to access all 10 hubs
                          weighted = FALSE,
                          grouping = "all",
...
# 4. SAVE RESULTS
save(lasso.rf.pre.full.CD.new_mastery50_DTccl4_cluster_1_2,
     lasso.rf.pre.full.CD.new_mastery50_plus_rna_DTccl4_cluster_1_2,
     file = "Models_mastery50_DTccl4_cluster_1_2.RData")

# load("C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/New_Data/Cluster_Modelierung/Intra_new/Models_mastery50_DTccl4_cluster_1_2.RData")
# 
# # 5. EVALUATION AND SUMMARY STATISTICS
# res.m10 <- gof.all.complete(model = lasso.rf.pre.full.CD.new_mastery50_DTccl4_cluster_1_2, prediction.method = "lasso")
# cat("\n=== MASTERY 10 PROTEIN CORRELATIONS ===\n")
# cor_m10 <- sapply(lasso.rf.pre.full.CD.new_mastery50_DTccl4_cluster_1_2, function(x) as.numeric(x$prediction.obj$model$correlation.pearson))
