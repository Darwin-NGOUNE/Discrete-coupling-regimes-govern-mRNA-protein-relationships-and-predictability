...
BDL_Full <- BDL_Full[Treatment %in% c('BDL', 'control')]

# Extract target proteins (pairs) directly from ClusterDiPa column
pairs.list.1.2.sub <- unique(BDL_Full[ClusterDiPa %in% c(1, 2), GeneProtein])
...
lasso.rf.pre.full.CD.new_mastery50_LCPM_cluster_1_2_sub <- pblapply(FUN = protein.regression.complete,
                          pairs.list.1.2.sub,
...
lasso.rf.pre.full.CD.new_mastery50_plus_rna_LCPM_cluster_1_2_sub <- pblapply(FUN = protein.regression.complete,
                          pairs.list.1.2.sub,
                          dataset = BDL_Full, # <-- Full dataset to access all 10 hubs
                          weighted = FALSE,
                          grouping = "all",
...
# 4. SAVE RESULTS
save(lasso.rf.pre.full.CD.new_mastery50_LCPM_cluster_1_2_sub,
     lasso.rf.pre.full.CD.new_mastery50_plus_rna_LCPM_cluster_1_2_sub,
     file = "Models_mastery50_LCPM_cluster_1_2_sub.RData")

# 5. EVALUATION AND SUMMARY STATISTICS
# res.m10 <- gof.all.complete(model = lasso.rf.pre.full.CD.new_mastery50_LCPM_cluster_1_2_sub, prediction.method = "lasso")
# cat("\n=== MASTERY 10 PROTEIN CORRELATIONS ===\n")
# cor_m10 <- sapply(lasso.rf.pre.full.CD.new_mastery50_LCPM_cluster_1_2_sub, function(x) as.numeric(x$prediction.obj$model$correlation.pearson))
