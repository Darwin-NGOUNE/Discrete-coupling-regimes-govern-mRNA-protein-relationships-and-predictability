...
pairs.list.3.4 <- unique(BDL_Full[ClusterDiPa %in% c(3, 4), GeneProtein])
...
lasso.rf.pre.full.CD.new_mastery50_LCPM_cluster_3_4 <- pblapply(FUN = protein.regression.complete,
                          pairs.list.3.4,
...
lasso.rf.pre.full.CD.new_mastery50_plus_rna_LCPM_cluster_3_4 <- pblapply(FUN = protein.regression.complete,
                          pairs.list.3.4,
                          dataset = BDL_Full, # <-- Full dataset to access all 10 hubs
                          weighted = FALSE,
                          grouping = "all",
...
# 4. SAVE RESULTS
save(lasso.rf.pre.full.CD.new_mastery50_LCPM_cluster_3_4,
     lasso.rf.pre.full.CD.new_mastery50_plus_rna_LCPM_cluster_3_4,
     file = "Models_mastery50_LCPM_cluster_3_4.RData")

# 5. EVALUATION AND SUMMARY STATISTICS
# res.m10 <- gof.all.complete(model = lasso.rf.pre.full.CD.new_mastery50_LCPM_cluster_3_4, prediction.method = "lasso")
# cat("\n=== MASTERY 10 PROTEIN CORRELATIONS ===\n")
# cor_m10 <- sapply(lasso.rf.pre.full.CD.new_mastery50_LCPM_cluster_3_4, function(x) as.numeric(x$prediction.obj$model$correlation.pearson))
