...
pairs.list.5.6.sub <- unique(BDL_Full[ClusterDiPa %in% c(5, 6), GeneProtein])
...
lasso.rf.pre.full.CD.new_mastery50_LCPM_cluster_5_6_sub <- pblapply(FUN = protein.regression.complete,
                          pairs.list.5.6.sub,
...
lasso.rf.pre.full.CD.new_mastery50_plus_rna_LCPM_cluster_5_6_sub <- pblapply(FUN = protein.regression.complete,
                          pairs.list.5.6.sub,
                          dataset = BDL_Full, # <-- Full dataset to access all 10 hubs
                          weighted = FALSE,
                          grouping = "all",
...
# 4. SAVE RESULTS
save(lasso.rf.pre.full.CD.new_mastery50_LCPM_cluster_5_6_sub,
     lasso.rf.pre.full.CD.new_mastery50_plus_rna_LCPM_cluster_5_6_sub,
     file = "Models_mastery50_LCPM_cluster_5_6_sub.RData")

# 5. EVALUATION AND SUMMARY STATISTICS
# res.m10 <- gof.all.complete(model = lasso.rf.pre.full.CD.new_mastery50_LCPM_cluster_5_6_sub, prediction.method = "lasso")
# cat("\n=== MASTERY 10 PROTEIN CORRELATIONS ===\n")
# cor_m10 <- sapply(lasso.rf.pre.full.CD.new_mastery50_LCPM_cluster_5_6_sub, function(x) as.numeric(x$prediction.obj$model$correlation.pearson))
