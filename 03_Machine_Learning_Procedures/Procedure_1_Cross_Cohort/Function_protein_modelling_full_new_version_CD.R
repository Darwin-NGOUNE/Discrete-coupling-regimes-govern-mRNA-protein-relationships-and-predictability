...
protein.regression.baseline <- function(dataset,
  ...
  #sub_data <- dataset[GeneProtein == geneprotein, ] #For baseline merged data
...
lasso.mod <- function(Design,
  ...
  if (is.null(nfolds) && prediction.method == "full") {
    nfolds <- 10   # valeur par défaut si non précisé
  }
  ...
  if(prediction.method == "full"){
    ...
    loo.pred <- sapply(1:nrow(Design), function(x){
      ...
      lasso.obj$nfolds_used <- nrow(train.data)
    ...
    nfolds.vector <- unlist(loo.pred[3, ])
...
protein.regression.function <- function(
  ...
  weights.vector = NULL,
  model.method = "lasso", # lasso, svr, rf, lasso.rf.pre, adaptive.lasso OR BASELINE
  prediction.method = "loo", # loo, cv, full, random
