...
prot_bdl_cluster <- unique(BDL_Full[ClusterDiPa %in% c(5, 6), GeneProtein])
prot_ccl4_cluster <- unique(CCL4_Full[ClusterDiPa %in% c(5, 6), GeneProtein])
core_cluster_proteins <- intersect(prot_bdl_cluster, prot_ccl4_cluster)

cat("Number of target proteins (DiPa Cluster 5_6):", length(core_cluster_proteins), "\n")
...
rf_lasso_mastery50_proteins_BDL_CCL4_5_6_prime_sub <- pblapply(FUN = protein.regression.function,
                                                           pairs.list,
                                                           dataset   = BDL_Full,
                                                           newdata   = CCL4_Full,
                                                           weighted = FALSE,
...
rf_lasso_mastery50_plus_rna_BDL_CCL4_5_6_prime_sub <- pblapply(FUN = protein.regression.function,
                                                           pairs.list,
                                                           dataset   = BDL_Full,
                                                           newdata   = CCL4_Full,
                                                           weighted = FALSE,
...
# Save the models
output_file <- "C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/Cross_Data_Analysis/Funktionen_CD_Testing/Batch_corrected_prime_analysis/Mastery50/Models_mastery50_BDL_CCL4_5_6_prime_sub.RData"
save(rf_lasso_mastery50_proteins_BDL_CCL4_5_6_prime_sub,
     rf_lasso_mastery50_plus_rna_BDL_CCL4_5_6_prime_sub,
     file = output_file)
cat("\nModels saved successfully to:", output_file, "\n")

# 6. Analyze and print the result summary
rmse_bas <- sapply(rf_lasso_mastery50_proteins_BDL_CCL4_5_6_prime_sub, function(x) x$baseline.model$RMSE)
mae_bas <- sapply(rf_lasso_mastery50_proteins_BDL_CCL4_5_6_prime_sub, function(x) x$baseline.model$MAE)
cor_bas  <- sapply(rf_lasso_mastery50_proteins_BDL_CCL4_5_6_prime_sub, function(x) x$baseline.model$Pearson)

rmse_mod <- sapply(rf_lasso_mastery50_proteins_BDL_CCL4_5_6_prime_sub, function(x) x$model$rmse)
mae_mod <- sapply(rf_lasso_mastery50_proteins_BDL_CCL4_5_6_prime_sub, function(x) x$model$mae)
cor_mod  <- sapply(rf_lasso_mastery50_proteins_BDL_CCL4_5_6_prime_sub, function(x) x$model$correlation.pearson)
...
cat("\n=== SUMMARY TABLE (BDL_CCL4_Mastery50_5_6_prime_sub.R) ===\n")
print(results_table)
