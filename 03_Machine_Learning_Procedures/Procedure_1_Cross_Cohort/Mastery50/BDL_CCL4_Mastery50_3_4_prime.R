...
rf_lasso_mastery50_proteins_BDL_CCL4_3_4_prime <- pblapply(FUN = protein.regression.function,
                                                           pairs.list,
                                                           dataset   = BDL_Full,
                                                           newdata   = CCL4_Full,
                                                           weighted = FALSE,
...
rf_lasso_mastery50_plus_rna_BDL_CCL4_3_4_prime <- pblapply(FUN = protein.regression.function,
                                                           pairs.list,
                                                           dataset   = BDL_Full,
                                                           newdata   = CCL4_Full,
                                                           weighted = FALSE,
...
# Save the models
output_file <- "C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/Cross_Data_Analysis/Funktionen_CD_Testing/Batch_corrected_prime_analysis/Mastery50/Models_mastery50_BDL_CCL4_3_4_prime.RData"
save(rf_lasso_mastery50_proteins_BDL_CCL4_3_4_prime,
     rf_lasso_mastery50_plus_rna_BDL_CCL4_3_4_prime,
     file = output_file)
cat("\nModels saved successfully to:", output_file, "\n")

# 6. Analyze and print the result summary
rmse_bas <- sapply(rf_lasso_mastery50_proteins_BDL_CCL4_3_4_prime, function(x) x$baseline.model$RMSE)
mae_bas <- sapply(rf_lasso_mastery50_proteins_BDL_CCL4_3_4_prime, function(x) x$baseline.model$MAE)
cor_bas  <- sapply(rf_lasso_mastery50_proteins_BDL_CCL4_3_4_prime, function(x) x$baseline.model$Pearson)

rmse_mod <- sapply(rf_lasso_mastery50_proteins_BDL_CCL4_3_4_prime, function(x) x$model$rmse)
mae_mod <- sapply(rf_lasso_mastery50_proteins_BDL_CCL4_3_4_prime, function(x) x$model$mae)
cor_mod  <- sapply(rf_lasso_mastery50_proteins_BDL_CCL4_3_4_prime, function(x) x$model$correlation.pearson)
...
cat("\n=== SUMMARY TABLE (BDL_CCL4_Mastery50_3_4_prime.R) ===\n")
print(results_table)
