...
rf_lasso_full_rna_testing_Batch_prime_0_sub <- pblapply(FUN = protein.regression.function,
                                                          pairs.list,
                                                          dataset   = CCL4_Full,
                                                          newdata   = BDL_Full,
                                                          weighted = FALSE,
...
rf_lasso_full_proteins_testing_Batch_prime_0_sub  <- pblapply(FUN = protein.regression.function,
                                                                pairs.list,
                                                                dataset   = CCL4_Full,
                                                                newdata   = BDL_Full,
                                                                weighted = FALSE,
...
rf_lasso_full_combined_testing_Batch_prime_0_sub  <- pblapply(FUN = protein.regression.function,
                                                                pairs.list,
                                                                dataset   = CCL4_Full,
                                                                newdata   = BDL_Full,
                                                                weighted = FALSE,
...
# Save the models
output_file <- "C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/Cross_Data_Analysis/Funktionen_CD_Testing/Batch_corrected_prime_analysis/Models_rf_lasso_full_testing_Batch_prime_0_sub.RData"
save(rf_lasso_full_rna_testing_Batch_prime_0_sub ,
     rf_lasso_full_proteins_testing_Batch_prime_0_sub ,
     rf_lasso_full_combined_testing_Batch_prime_0_sub ,
     file = output_file)
...
load("C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/Cross_Data_Analysis/Funktionen_CD_Testing/Batch_corrected_prime_analysis/Models_rf_lasso_full_testing_Batch_prime_0_sub.RData")


######################rna#######################################################

cor_vals <- sapply(rf_lasso_full_rna_testing_Batch_prime_0_sub, function(x) {
  as.numeric(x$model$correlation.pearson)
})


sum(cor_vals < 0.5) #186
sum(cor_vals >= 0.5) #16
sum(cor_vals >= 0.8) #0
...
cor_vals1 <- sapply(rf_lasso_full_rna_testing_Batch_prime_0_sub, function(x) {
  as.numeric(x$baseline.model$Pearson)
})

sum(cor_vals1 < 0.5) #189
sum(cor_vals1 >= 0.5) #13
sum(cor_vals1 >= 0.8) #0

################proteins########################################################

cor_vals <- sapply(rf_lasso_full_proteins_testing_Batch_prime_0_sub, function(x) {
  as.numeric(x$model$correlation.pearson)
})

sum(cor_vals < 0.5) #114
sum(cor_vals >= 0.5) #88
sum(cor_vals >= 0.8) #37

##################combined######################################################

cor_vals <- sapply(rf_lasso_full_combined_testing_Batch_prime_0_sub, function(x) {
  as.numeric(x$model$correlation.pearson)
})

sum(cor_vals < 0.5) #128
sum(cor_vals >= 0.5) #74
sum(cor_vals >= 0.8) #35
...
rmse_bas <- sapply(rf_lasso_full_rna_testing_Batch_prime_0_sub , function(x) x$baseline.model$RMSE)
mae_bas <- sapply(rf_lasso_full_rna_testing_Batch_prime_0_sub , function(x) x$baseline.model$MAE)
cor_bas  <- sapply(rf_lasso_full_rna_testing_Batch_prime_0_sub , function(x) x$baseline.model$Pearson)

rmse_rna <- sapply(rf_lasso_full_rna_testing_Batch_prime_0_sub , function(x) x$model$rmse)
mae_rna <- sapply(rf_lasso_full_rna_testing_Batch_prime_0_sub , function(x) x$model$mae)
cor_rna  <- sapply(rf_lasso_full_rna_testing_Batch_prime_0_sub , function(x) x$model$correlation.pearson)

rmse_prot <- sapply(rf_lasso_full_proteins_testing_Batch_prime_0_sub , function(x) x$model$rmse)
mae_prot <- sapply(rf_lasso_full_proteins_testing_Batch_prime_0_sub , function(x) x$model$mae)
cor_prot  <- sapply(rf_lasso_full_proteins_testing_Batch_prime_0_sub , function(x) x$model$correlation.pearson)

rmse_comb <- sapply(rf_lasso_full_combined_testing_Batch_prime_0_sub , function(x) x$model$rmse)
mae_comb <- sapply(rf_lasso_full_combined_testing_Batch_prime_0_sub , function(x) x$model$mae)
cor_comb  <- sapply(rf_lasso_full_combined_testing_Batch_prime_0_sub , function(x) x$model$correlation.pearson)
...
1 Baseline 0.5752713 0.4430054   0.00312078
2      RNA 0.7386266 0.5816606  -0.01386946
3  Protein 0.6255283 0.4801201   0.23687691
4 Combined 0.6323546 0.4923399   0.20218590
