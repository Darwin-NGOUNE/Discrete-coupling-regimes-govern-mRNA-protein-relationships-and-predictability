...
rf_lasso_full_rna_testing_new_Batch_prime_5_6 <- pblapply(FUN = protein.regression.function,
                                                          pairs.list,
                                                          dataset   = BDL_Full,
                                                          newdata   = CCL4_Full,
                                                          weighted = FALSE,
...
rf_lasso_full_proteins_testing_new_Batch_prime_5_6 <- pblapply(FUN = protein.regression.function,
                                                               pairs.list,
                                                               dataset   = BDL_Full,
                                                               newdata   = CCL4_Full,
                                                               weighted = FALSE,
...
rf_lasso_full_combined_testing_new_Batch_prime_5_6 <- pblapply(FUN = protein.regression.function,
                                                               pairs.list,
                                                               dataset   = BDL_Full,
                                                               newdata   = CCL4_Full,
                                                               weighted = FALSE,
...
# Save the models
output_file <- "C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/Cross_Data_Analysis/Funktionen_CD_Testing/Batch_corrected_prime_analysis/Models_rf_lasso_full_testing_new_Batch_prime_5_6.RData"
save(rf_lasso_full_rna_testing_new_Batch_prime_5_6,
     rf_lasso_full_proteins_testing_new_Batch_prime_5_6,
     rf_lasso_full_combined_testing_new_Batch_prime_5_6,
     file = output_file)
...
load("C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/Cross_Data_Analysis/Funktionen_CD_Testing/Batch_corrected_prime_analysis/Models_rf_lasso_full_testing_new_Batch_prime_5_6.RData")


######################rna#######################################################

cor_vals <- sapply(rf_lasso_full_rna_testing_new_Batch_prime_5_6, function(x) {
...
cor_vals1 <- sapply(rf_lasso_full_rna_testing_new_Batch_prime_5_6, function(x) {
  as.numeric(x$baseline.model$Pearson)
})

sum(cor_vals1 < 0.5) #23
sum(cor_vals1 >= 0.5) #0
sum(cor_vals1 >= 0.8) #0

################proteins########################################################

cor_vals <- sapply(rf_lasso_full_proteins_testing_new_Batch_prime_5_6, function(x) {
  as.numeric(x$model$correlation.pearson)
})
...
cor_vals <- sapply(rf_lasso_full_combined_testing_new_Batch_prime_5_6, function(x) {
  as.numeric(x$model$correlation.pearson)
})
...
rmse_bas <- sapply(rf_lasso_full_rna_testing_new_Batch_prime_5_6, function(x) x$baseline.model$RMSE)
mae_bas <- sapply(rf_lasso_full_rna_testing_new_Batch_prime_5_6, function(x) x$baseline.model$MAE)
cor_bas  <- sapply(rf_lasso_full_rna_testing_new_Batch_prime_5_6, function(x) x$baseline.model$Pearson)

rmse_rna <- sapply(rf_lasso_full_rna_testing_new_Batch_prime_5_6, function(x) x$model$rmse)
mae_rna <- sapply(rf_lasso_full_rna_testing_new_Batch_prime_5_6, function(x) x$model$mae)
cor_rna  <- sapply(rf_lasso_full_rna_testing_new_Batch_prime_5_6, function(x) x$model$correlation.pearson)

rmse_prot <- sapply(rf_lasso_full_proteins_testing_new_Batch_prime_5_6, function(x) x$model$rmse)
mae_prot <- sapply(rf_lasso_full_proteins_testing_new_Batch_prime_5_6, function(x) x$model$mae)
cor_prot  <- sapply(rf_lasso_full_proteins_testing_new_Batch_prime_5_6, function(x) x$model$correlation.pearson)

rmse_comb <- sapply(rf_lasso_full_combined_testing_new_Batch_prime_5_6, function(x) x$model$rmse)
mae_comb <- sapply(rf_lasso_full_combined_testing_new_Batch_prime_5_6, function(x) x$model$mae)
cor_comb  <- sapply(rf_lasso_full_combined_testing_new_Batch_prime_5_6, function(x) x$model$correlation.pearson)
...
1 Baseline 0.5366613 0.4152806 -0.027176378
2      RNA 0.7254714 0.5898444  0.006978667
3  Protein 0.5393582 0.4194671  0.191842419
4 Combined 0.6307728 0.4919891  0.104157233
