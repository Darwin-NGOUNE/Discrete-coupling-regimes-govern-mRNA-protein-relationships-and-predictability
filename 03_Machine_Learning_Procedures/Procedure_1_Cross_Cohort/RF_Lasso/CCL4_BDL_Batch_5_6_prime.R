...
rf_lasso_full_rna_testing_Batch_prime_5_6 <- pblapply(FUN = protein.regression.function,
                                                      pairs.list,
                                                      dataset   = CCL4_Full,
                                                      newdata   = BDL_Full,
                                                      weighted = FALSE,
...
rf_lasso_full_proteins_testing_Batch_prime_5_6 <- pblapply(FUN = protein.regression.function,
                                                           pairs.list,
                                                           dataset   = CCL4_Full,
                                                           newdata   = BDL_Full,
                                                           weighted = FALSE,
...
rf_lasso_full_combined_testing_Batch_prime_5_6 <- pblapply(FUN = protein.regression.function,
                                                           pairs.list,
                                                           dataset   = CCL4_Full,
                                                           newdata   = BDL_Full,
                                                           weighted = FALSE,
...
# Save the models
output_file <- "C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/Cross_Data_Analysis/Funktionen_CD_Testing/Batch_corrected_prime_analysis/Models_rf_lasso_full_testing_Batch_prime_5_6.RData"
save(rf_lasso_full_rna_testing_Batch_prime_5_6,
     rf_lasso_full_proteins_testing_Batch_prime_5_6,
     rf_lasso_full_combined_testing_Batch_prime_5_6,
     file = output_file)
...
load("C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/Cross_Data_Analysis/Funktionen_CD_Testing/Batch_corrected_prime_analysis/Models_rf_lasso_full_testing_Batch_prime_5_6.RData")


######################rna#######################################################

cor_vals <- sapply(rf_lasso_full_rna_testing_Batch_prime_5_6, function(x) {
...
cor_vals1 <- sapply(rf_lasso_full_rna_testing_Batch_prime_5_6, function(x) {
...
cor_vals <- sapply(rf_lasso_full_proteins_testing_Batch_prime_5_6, function(x) {
  as.numeric(x$model$correlation.pearson)
})
...
cor_vals <- sapply(rf_lasso_full_combined_testing_Batch_prime_5_6, function(x) {
  as.numeric(x$model$correlation.pearson)
})
...
rmse_bas <- sapply(rf_lasso_full_rna_testing_Batch_prime_5_6, function(x) x$baseline.model$RMSE)
mae_bas <- sapply(rf_lasso_full_rna_testing_Batch_prime_5_6, function(x) x$baseline.model$MAE)
cor_bas  <- sapply(rf_lasso_full_rna_testing_Batch_prime_5_6, function(x) x$baseline.model$Pearson)

rmse_rna <- sapply(rf_lasso_full_rna_testing_Batch_prime_5_6, function(x) x$model$rmse)
mae_rna <- sapply(rf_lasso_full_rna_testing_Batch_prime_5_6, function(x) x$model$mae)
cor_rna  <- sapply(rf_lasso_full_rna_testing_Batch_prime_5_6, function(x) x$model$correlation.pearson)

rmse_prot <- sapply(rf_lasso_full_proteins_testing_Batch_prime_5_6, function(x) x$model$rmse)
mae_prot <- sapply(rf_lasso_full_proteins_testing_Batch_prime_5_6, function(x) x$model$mae)
cor_prot  <- sapply(rf_lasso_full_proteins_testing_Batch_prime_5_6, function(x) x$model$correlation.pearson)

rmse_comb <- sapply(rf_lasso_full_combined_testing_Batch_prime_5_6, function(x) x$model$rmse)
mae_comb <- sapply(rf_lasso_full_combined_testing_Batch_prime_5_6, function(x) x$model$mae)
cor_comb  <- sapply(rf_lasso_full_combined_testing_Batch_prime_5_6, function(x) x$model$correlation.pearson)
...
1 Baseline 0.5318906 0.4224664  -0.04589655
2      RNA 0.6483532 0.5142680   0.02955535
3  Protein 0.5532903 0.4479404   0.37291166
4 Combined 0.5045409 0.4046142   0.39720373
