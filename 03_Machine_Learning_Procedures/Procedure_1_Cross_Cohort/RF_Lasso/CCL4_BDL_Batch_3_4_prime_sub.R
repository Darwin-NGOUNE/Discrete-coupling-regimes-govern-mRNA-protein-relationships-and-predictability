...
rf_lasso_full_rna_testing_Batch_prime_3_4_sub <- pblapply(FUN = protein.regression.function,
                                                          pairs.list,
                                                          dataset   = CCL4_Full,
                                                          newdata   = BDL_Full,
                                                          weighted = FALSE,
...
rf_lasso_full_proteins_testing_Batch_prime_3_4_sub  <- pblapply(FUN = protein.regression.function,
                                                                pairs.list,
                                                                dataset   = CCL4_Full,
                                                                newdata   = BDL_Full,
                                                                weighted = FALSE,
...
rf_lasso_full_combined_testing_Batch_prime_3_4_sub  <- pblapply(FUN = protein.regression.function,
                                                                pairs.list,
                                                                dataset   = CCL4_Full,
                                                                newdata   = BDL_Full,
                                                                weighted = FALSE,
...
# Save the models
output_file <- "C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/Cross_Data_Analysis/Funktionen_CD_Testing/Batch_corrected_prime_analysis/Models_rf_lasso_full_testing_Batch_prime_3_4_sub.RData"
save(rf_lasso_full_rna_testing_Batch_prime_3_4_sub ,
     rf_lasso_full_proteins_testing_Batch_prime_3_4_sub ,
     rf_lasso_full_combined_testing_Batch_prime_3_4_sub ,
     file = output_file)
...
load("C:/Users/ngoune/Documents/Projet I/Protein_Modeling_share/Cross_Data_Analysis/Funktionen_CD_Testing/Batch_corrected_prime_analysis/Models_rf_lasso_full_testing_Batch_prime_3_4_sub.RData")


######################rna#######################################################

cor_vals <- sapply(rf_lasso_full_rna_testing_Batch_prime_3_4_sub, function(x) {
...
cor_vals1 <- sapply(rf_lasso_full_rna_testing_Batch_prime_3_4_sub, function(x) {
  as.numeric(x$baseline.model$Pearson)
})

sum(cor_vals1 < 0.5) #40
sum(cor_vals1 >= 0.5) #27
sum(cor_vals1 >= 0.8) #1

################proteins########################################################

cor_vals <- sapply(rf_lasso_full_proteins_testing_Batch_prime_3_4_sub, function(x) {
  as.numeric(x$model$correlation.pearson)
})
...
cor_vals <- sapply(rf_lasso_full_combined_testing_Batch_prime_3_4_sub, function(x) {
  as.numeric(x$model$correlation.pearson)
})
...
rmse_bas <- sapply(rf_lasso_full_rna_testing_Batch_prime_3_4_sub , function(x) x$baseline.model$RMSE)
mae_bas <- sapply(rf_lasso_full_rna_testing_Batch_prime_3_4_sub , function(x) x$baseline.model$MAE)
cor_bas  <- sapply(rf_lasso_full_rna_testing_Batch_prime_3_4_sub , function(x) x$baseline.model$Pearson)

rmse_rna <- sapply(rf_lasso_full_rna_testing_Batch_prime_3_4_sub , function(x) x$model$rmse)
mae_rna <- sapply(rf_lasso_full_rna_testing_Batch_prime_3_4_sub , function(x) x$model$mae)
cor_rna  <- sapply(rf_lasso_full_rna_testing_Batch_prime_3_4_sub , function(x) x$model$correlation.pearson)

rmse_prot <- sapply(rf_lasso_full_proteins_testing_Batch_prime_3_4_sub , function(x) x$model$rmse)
mae_prot <- sapply(rf_lasso_full_proteins_testing_Batch_prime_3_4_sub , function(x) x$model$mae)
cor_prot  <- sapply(rf_lasso_full_proteins_testing_Batch_prime_3_4_sub , function(x) x$model$correlation.pearson)

rmse_comb <- sapply(rf_lasso_full_combined_testing_Batch_prime_3_4_sub , function(x) x$model$rmse)
mae_comb <- sapply(rf_lasso_full_combined_testing_Batch_prime_3_4_sub , function(x) x$model$mae)
cor_comb  <- sapply(rf_lasso_full_combined_testing_Batch_prime_3_4_sub , function(x) x$model$correlation.pearson)
...
1 Baseline 0.7788210 0.6192649    0.2351303
2      RNA 0.9110039 0.7146612    0.2221258
3  Protein 0.7547444 0.5904281    0.5156890
4 Combined 0.7925843 0.6277880    0.4074887
