...
design.function <- function(dataset,
  ...
  if(RNA.log.scale == TRUE){
    glong <- melt(sub_data[, .(MiceInfo, Treatment, TreatmentTime, GeneProtein, log2(GeneCount + 1))], id.vars = c("MiceInfo", "Treatment", "TreatmentTime", "GeneProtein"))
  }else{
    glong <- melt(sub_data[, .(MiceInfo, Treatment, TreatmentTime, GeneProtein, GeneCount)], id.vars = c("MiceInfo", "Treatment", "TreatmentTime", "GeneProtein"))
    
  }
  
  gwide <- dcast(glong, MiceInfo + Treatment + TreatmentTime  ~ GeneProtein)
  
  plong <- melt(sub_data[, .(MiceInfo, TreatmentTotal, GeneProtein, ProteinIntensity)], id.vars = c("MiceInfo", "TreatmentTotal", "GeneProtein"))
  pwide <- dcast(plong, MiceInfo + TreatmentTotal ~ GeneProtein)
  ...
  colsg <- !(colnames(gwide) %in% c("MiceInfo","Treatment","TreatmentTime"))
  gdesign <- as.data.table(subset(gwide,,colsg))
  
  colsp <- !(colnames(pwide) %in% c("MiceInfo","TreatmentTotal"))
  pdesign <- as.data.table(subset(pwide,,colsp))
  ...
  if(design.m == "genes"){ # RNA as covariates
    ...
    pdesign.raw <- as.data.table(pdesign)[,paste0(protein, "_P"):=NULL]
  ...
  treatment_temp <- factor(pwide$TreatmentTotal,
                           levels = c("month0_oil", 
                                      "month2_oil",  
                                      "month12_oil", 
                                      "month2_ccl4",
  ...
  experiments <- factor(treatment_temp[i_rel],
                        levels = c("month0_oil", 
                                   "month2_oil",  
                                   "month12_oil", 
                                   "month2_ccl4",
  ...
  # treatment <- factor(str_split(treatment_temp[i_rel], "_", simplify = TRUE)[,2],
  #                     levels = c("oil", "ccl4"))
