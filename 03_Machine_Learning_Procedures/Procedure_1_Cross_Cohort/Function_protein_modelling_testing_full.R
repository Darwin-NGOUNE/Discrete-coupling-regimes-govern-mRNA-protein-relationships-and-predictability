...
protein.regression.baseline <- function(dataset,
  ...
  sub_data$TreatmentTotal <- factor(sub_data$TreatmentTotal,
                                    levels = c("month0_oil", "month2_oil", "month12_oil",
  ...
  # 4. Prédictions sur newdata (BUG FIXED)
  newdata_sub <- newdata[newdata$GeneProtein == geneprotein, ]
  
  # Suppression sécurisée des NAs pour correspondre au N exact
  #newdata_sub <- newdata_sub[complete.cases(newdata_sub[, c("ProteinIntensity", "GeneCount")]), ]
...
#   newdat <- get_all_vars(form.obj, newdata)
#   prediction <- predict(gam.obj, newdat)
#   
#   # y = valeurs observées dans newdata
#   y_test <- newdata$ProteinIntensity
#   
#   # 5. Calcul des métriques
#   rmse <- sqrt(mean((y_test - prediction)^2))
#   mae <- mean(abs(y_test - prediction))
#   pearson_cor <- cor(y_test, prediction, method = "pearson")
#   spearman_cor <- cor(y_test, prediction, method = "spearman")
#   
#   # 6. Retourner les résultats
#   gam_list <- list(regression.object = gam.obj, 
#                    gene = gene,
...
lasso.mod <- function(Design,
  ...
  # Étape 3 : créer une liste avec les variables sélectionnées
...
rf.preselect <- function(Design,          # matrice des covariables (X)
  ...
  fit.ranger <- ranger(x = Design, y = y, data = DTrf,
                       ...
                       min.node.size = 5)                  # taille min des feuilles
...
protein.regression.function <- function(
  ...
  Design.test.obj <- design.function.new(dataset = newdata,
                                     geneprotein = geneprotein,
                                     weighted = weighted,
                                     weight.method = weight.method,
                                     weight.matrix = weight.matrix,
  ...
  if(model.method == "baseline.only"){
    ...
    model <- lasso.mod(Design = Design.train,
                       y = y.train,
                       y.t = y.test,
                       w = w.train,
                       n = n.train,
    ...
    model <- rf.mod(Design = Design.train,
                    y = y.train,
                    y.t = y.test,
                    n = n.train,
                    gene = gene.train,
